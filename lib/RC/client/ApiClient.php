...
class ApiClient {
  ...
  function __construct($platform) {
    ...
    //if ($host === null) {
    //  $this->host = 'https://platform.devtest.ringcentral.com/restapi';
    //} else {
    //  $this->host = $host;
    //}
  }
   ...
   * @return mixed
   */
  public function callApiXXX($resourcePath, $method, $queryParams, $postData,
  ...
  public function callAPI($resourcePath, $method, $queryParams, $postData, $headerParams, $authSettings) {
    ...
    $resourcePath = '/restapi' . $resourcePath;

    $response = null;

    switch ($method) {
      case (self::$POST): $response =  $this->platform->post($resourcePath, $queryParams, $postData); break; //, $headerParams
      case (self::$PUT): $response =  $this->platform->put($resourcePath, $queryParams, $postData); break; //, $headerParams
      case (self::$DELETE): $response =  $this->platform->delete($resourcePath, $queryParams, $postData); break; //, $headerParams
      default: $response =  $this->platform->get($resourcePath, $queryParams); break; //, $headerParams
    }
  ...
  public function validateParameter($options, $paramName, $nickname){
  ...
  public function parseQueryParam($queryParams, $options, $paramName, $baseName){
    if(isset($options[$paramName])) {
        $queryParams[$baseName] = $this->toQueryValue($options[$paramName]);
    }
    return $queryParams;
  }

  public function parseHeaderParam($headerParams, $options, $paramName, $baseName){
    if(isset($options[$paramName])) {
      $headerParams[$baseName] = $this->toHeaderValue($options[$paramName]);
    }
    return $headerParams;
  }

  public function parsePathParam($resourcePath, $options, $paramName, $baseName){
    if(isset($options[$paramName])) {
      $resourcePath = str_replace("{" . $baseName . "}", $this->toPathValue($options[$paramName]), $resourcePath);
    }
    return $resourcePath;
  }

  public function parseFormParam($formParams, $options, $paramName, $baseName, $isFile = false){
    if (isset($options[$paramName])) {
      $formParams[$baseName] = ($isFile ? '@' : '') . $this->toFormValue($options[$paramName]);
    }
    return $formParams;
  }
